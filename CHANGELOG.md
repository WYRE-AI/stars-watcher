...
- **Star counting silently ignored `GH_API_TOKEN`.** `fetch_repo_stars` and `fetch_latest_releases` always used the default `GITHUB_TOKEN`, which can only see *public* repos in the org — the 48 private repos (out of 166 total) were invisible to it, undercounting the estate with no error. Both now use the new `_best_token()` helper, which prefers the admin-scoped `GH_API_TOKEN` PAT when it's configured. This doesn't add coverage by itself — `GH_API_TOKEN` still needs a valid PAT (see clone-traffic note below) — but once one exists, star counts and release lookups pick up private repos automatically.
